...
def _ensure_paid_plan(user_id: str) -> None:
    ...
    plan = (profile.data[0].get("plan") if profile.data else "free") or "free"
    if plan == "free":
        raise HTTPException(
            status_code=402,
            detail={
...
async def create_api_key(body: dict, user: dict = Depends(get_current_user)):
    _ensure_paid_plan(user["id"])

    name = (body.get("name") or "Default Key").strip()
    scopes = body.get("scopes") or ["read", "generate"]
    expires_at = body.get("expires_at")

    if not isinstance(scopes, list) or any(scope not in {"read", "generate"} for scope in scopes):
        raise HTTPException(status_code=400, detail="Invalid scopes")

    raw = f"sk_live_{secrets.token_urlsafe(36)}"
    key_prefix = raw[:12]
...
async def revoke_api_key(api_key_id: str, user: dict = Depends(get_current_user)):
    ...
    supabase.table("api_keys").update(
        {"is_active": False, "last_used_at": datetime.now(timezone.utc).isoformat()}
